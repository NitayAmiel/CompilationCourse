...
namespace output {
    ...
    void CodeBuffer::insert_str_divide_by_0(std::string reg_name, std::string error_msg){
        ...
        res =  tmp_error_msg_reg + " = getelementptr  [" +  std::to_string(MESSAGE_ERROR_SIZE) + " x i8], [" + std::to_string(MESSAGE_ERROR_SIZE) + " x i8]* " + error_msg + ", i32 0, i32 0\n";
    ...
    static std::string _op_to_string(ast::RelOpType type) {
        switch (type) {
    ...
    static std::string op_to_string(ast::RelOpType type) {
        return _op_to_string(type) + " ";
    }

    static std::string op_to_string_and_sign(ast::RelOpType op, ast::BuiltInType type) {
        if(op == ast::RelOpType::EQ || op == ast::RelOpType::NE){
            return _op_to_string(op) + " ";
        }
        std::string singed_letter = type == ast::BuiltInType::BYTE ? "u" : "s";
        return singed_letter + _op_to_string(op) + " ";
    }
    ...
    static int is_int_byte (ast::BuiltInType x, ast::BuiltInType y){
        if (x == ast::BuiltInType::INT && y == ast::BuiltInType::BYTE){
            return 1;
        }
        if( y == ast::BuiltInType::INT && x == ast::BuiltInType::BYTE){
            return 2;
        }
        return 0;
    }
    ...
    static bool is_byte_or_bool(ast::BuiltInType x){
        return (x == ast::BuiltInType::BOOL ||  x == ast::BuiltInType::BYTE);
    }
        ...
        if(id_entry ) {
            ...
            if(node.type != ast::BuiltInType::DEF_AS_FUNC) {
                if(defined_as_parameter(*id_entry)){
                    ...
                    // if(is_byte_or_bool(id_entry->ret_type)){
                    //     node.reg = this->code_buffer.freshVar();
                    //     this->code_buffer.emit(node.reg + " = zext " + get_type_string(node.type) + get_register_name(*id_entry) + " to i32");
                    // }
...
// Nitay 
    ...
    void MyVisitor::visit(ast::Break &node) {
        ...
        code_buffer.emit("br label " + this->exit_labels.at(this->exit_labels.size()-1));
    }
    ...
    void MyVisitor::visit(ast::Continue &node) {
        ...
        code_buffer.emit("br label " + this->loop_head_labels.at(this->loop_head_labels.size()-1));
    }
    ...
    void MyVisitor::visit(ast::While &node) {
        ...
        std::string loop_head_label = this->code_buffer.freshLabel();
        this->loop_head_labels.push_back(loop_head_label);
        std::string exit_label = this->code_buffer.freshLabel();
        this->exit_labels.push_back(exit_label);
        ...
        this->end_scope();
        this->loop_head_labels.pop_back();
        this->exit_labels.pop_back();
        this->in_while--;
